...
module "tfe_workspace" {
  ...
  source    = "github.com/svashisht03/terraform-aws-mcaf-workspace"
...
module "additional_tfe_workspaces" {
  ...
  permissions_boundary           = templatefile(var.permissions_boundary, { account_id = module.account.id })
  ...
  workload_boundary              = templatefile(var.workload_boundary, { account_id = module.account.id })
  workload_boundary_name         = var.workload_boundary_name
}
